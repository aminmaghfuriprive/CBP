...
import { RoleManagementView } from '../../../src/components/employees/organisms/RoleManagementView';
import { useData } from '@cbp/core';
import { Users, Briefcase, ShieldCheck, Clock, Wallet, Lock } from 'lucide-react';

export default function EmployeesPage() {
  const { employees } = useData();
  const [activeTab, setActiveTab] = useState<'directory' | 'attendance' | 'payroll' | 'roles'>('directory');
  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto">
      ...
      <div className="flex gap-4 border-b border-slate-200 dark:border-slate-800 mb-6 overflow-x-auto">
        ...
        >
          <Lock className="h-4 w-4" /> Role & Akses
          {activeTab === 'roles' && <div className="absolute bottom-0 left-0 w-full h-0.5 bg-cbp-gold rounded-t-full"></div>}
        </button>
      </div>
