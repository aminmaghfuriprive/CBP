...
import { LeadPipelineView } from '../../../src/components/marketing/LeadPipelineView';
...
export default function MarketingPage() {
  ...
  const [activeTab, setActiveTab] = useState<'leads' | 'blast' | 'inbox'>('leads');

  return (
    <div className="h-full flex flex-col">
      <PageHeader 
        title="Marketing Command Center" 
        subtitle="Pusat pengelolaan Leads, Kampanye Sosial Media, dan Komunikasi Klien." 
      />
      ...
      <div className="flex gap-4 border-b border-slate-200 dark:border-slate-800 mb-6 flex-shrink-0 overflow-x-auto">
        ...
        >
          <Target className="h-4 w-4" /> Pipeline Leads
          {activeTab === 'leads' && <div className="absolute bottom-0 left-0 w-full h-0.5 bg-cbp-gold rounded-t-full"></div>}
        </button>
        ...
        >
          <Rocket className="h-4 w-4" /> Campaigns (Blast)
